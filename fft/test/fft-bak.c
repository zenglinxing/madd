...
static void test_fft_ifft_consistency() {
    ...
    for (uint64_t i = 0; i < n; i++) {
        ...
        if (real_err > TOLERANCE || imag_err > TOLERANCE) {
            ...
            printf("i=%llu\t%f %f\t%f %f\n", i, original[i].real, original[i].imag, input[i].real, input[i].imag);
            break;
        }
    }
...
static void test_sinusoidal_fft() {
    ...
    // Generate sinusoidal signal: x(t) = sin(2πft)
    for (uint64_t i = 0; i < n; i++) {
        double t = (double)i / n;
    ...
    uint64_t expected_bin = (uint64_t)freq;
    uint64_t expected_bin_alt = n - expected_bin;
    
    int passed = (peak_idx == expected_bin || 
                 peak_idx == expected_bin_alt ||
                 abs((int)peak_idx - (int)expected_bin) <= 1 ||
                 abs((int)peak_idx - (int)expected_bin_alt) <= 1);

    if (!passed) {
        printf("  Expected peak at index %llu or %llu, but found at %llu\n",
               (unsigned long long)expected_bin, 
               (unsigned long long)expected_bin_alt,
               (unsigned long long)peak_idx);
        
        printf("  FFT Magnitudes:\n");
        for (uint64_t i = 0; i < n/2 + 1; i++) {
            printf("    Bin %llu: %.4f\n", (unsigned long long)i, Cnum_Radius(input[i]));
        }
    }
...
static void test_non_power_of_2() {
    ...
    const double scale_factor = (double)expected_ceil / n;
    for (uint64_t i = 0; i < n; i++) {
        double expected = original_reals[i];
        double err = fabs(input[i].real - expected);
        if (err > TOLERANCE) {
            passed = 0;
            printf("  Mismatch at index %llu: %.6f (expected %.6f, error=%e)\n",
                   (unsigned long long)i, input[i].real, original_reals[i], err);
            break;
        }
    }
