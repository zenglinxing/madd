...
{ 
    ...
    void *space=(void*)malloc(nx*(sizeof(struct Fmin_NM_Element)+2*sizeof(RB_Tree_Node)+sizeof(struct Fmin_NM_Element))+6*size_cpy); 
    /* NM parameter */ 
    ...
    register double *x_mean=x_list, *x_sum=x_list+n_param, *xr=x_list+2*n_param /* 2*x_mean+x_h */,yr; /* xh is nm.max->x */ 
    register double *xe=x_list+3*n_param /* 3*x_mean-2*xh */,ye, *xoc=x_list+4*n_param /* 1.5*x_mean-0.5*xh */,yoc; 
    register double *xic=x_list+5*n_param,yic /* 1.5*x_mean+0.5*xh */; 
    /* Search */ 
    ...
    for (i_step=0; i_step<n_step; i_step++){ 
        ...
        if (print_step && print_next_step==i_step){
            ...
            for (ix=0,pnme=nm.element; ix<nx; ix++,pnme++){
                for (i_param=0; i_param<n_param; i_param++){
                    printf("%e\t",(double)pnme->x[i_param]);
                }
