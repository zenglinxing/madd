...
#define MATRIX_HERMITIAN_TRANSPOSE__ALGORITHM(Cnum, Matrix_Transpose_c64) \
{ \
    if (matrix == NULL){ \
        wchar_t error_info[MADD_ERROR_INFO_LEN]; \
        swprintf(error_info, MADD_ERROR_INFO_LEN, L"%hs: matrix is NULL.", __func__); \
 ...
 \
    Matrix_Transpose_c64(m, n, matrix); \
    Cnum *p = matrix; \
    for (uint64_t i=0; i<m; i++){ \
        for (uint64_t j=0; j<n; j++,p++){ \
            p->imag = -p->imag; \
        } \
    } \
} \
...
MATRIX_HERMITIAN_TRANSPOSE__ALGORITHM(Cnum, Matrix_Transpose_c64)

bool Matrix_Hermitian_Transpose_c32(uint64_t m, uint64_t n, Cnum32 *matrix)
MATRIX_HERMITIAN_TRANSPOSE__ALGORITHM(Cnum32, Matrix_Transpose_c32)

bool Matrix_Hermitian_Transpose_cl(uint64_t m, uint64_t n, Cnuml *matrix)
MATRIX_HERMITIAN_TRANSPOSE__ALGORITHM(Cnuml, Matrix_Transpose_cl)

#ifdef ENABLE_QUADPRECISION
bool Matrix_Hermitian_Transpose_c128(uint64_t m, uint64_t n, Cnum128 *matrix)
MATRIX_HERMITIAN_TRANSPOSE__ALGORITHM(Cnum128, Matrix_Transpose_c128)
#endif /* ENABLE_QUADPRECISION */