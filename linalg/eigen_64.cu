                  ...
                  bool flag_right, Cnum *eigenvector_right)
{
    ...
    size_t size_matrix = nn * sizeof(double), size_nn = nn * sizeof(Cnum), size_n = (uint64_t)n * sizeof(Cnum);
    ...
    size_t size_nn_matrix = (uint64_t)n*n*sizeof(double), size_vl_vr = 0;
    if (flag_left) size_vl_vr += size_nn_matrix;
    if (flag_right) size_vl_vr += size_nn_matrix;
    double *vl = (double*)malloc(size_vl_vr), *vr = (flag_left) ? vl + nn : vl;
    if (vl == NULL){
        cudaFree(d_matrix);
        wchar_t error_info[MADD_ERROR_INFO_LEN];
                      ...
                      bool flag_right, Cnum *eigenvector_right)
{
    ...
    uint64_t nn = (uint64_t)n * n;
    size_t size_matrix = nn * sizeof(Cnum), size_nn = nn * sizeof(Cnum), size_n = (uint64_t)n * sizeof(Cnum);
    size_t size_alloc = size_matrix + size_n;
    if (flag_left) size_alloc += size_nn;
    if (flag_right) size_alloc += size_nn;
    Cnum *d_matrix;
