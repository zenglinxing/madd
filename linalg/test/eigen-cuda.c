...
void NxN_Real_Matrix_Test()
{
    ...
    bool ret_eigen = Eigen_cuda64(n, matrix, eigenvalue, false, vl, true, vr);
    if (ret_eigen == false){
        flag_real_left_verified = flag_real_right_verified = false;
        printf("*** internal error from cuda function. ***\n");
        return;
    }
...
void _2x2_Real_Matrix_Complex_Eigen_Test()
{
    ...
    Eigen_cuda64(2, mat2, eig2, false, vl2, true, vr2);
    printf("Eigenvalues\n");
