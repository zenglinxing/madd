...
void _3x3_det0_matrix_test()
{
    ...
    printf("\n");
    bool flag_blas = Determinant_cuda(3, mat1, &res1);
    bool flag_Bareiss = Determinant_Bareiss(3, mat2, &res2);
    if (!flag_blas){
        flag_success = false;
        printf("*** Determinant CUDA doesn't work ***\n");
    }
    if (!flag_Bareiss){
        flag_success = false;
    ...
    if (fabs(res1) > tolerance){
        flag_success = false;
        printf("*** Determinant CUDA is not 0 -> %f ***\n", res1);
    }
...
void random_matrix_test()
{
    ...
    double res1, res2;
    bool flag_blas = Determinant_cuda(n_random_test, mat1, &res1);
    bool flag_Bareiss = Determinant_Bareiss(n_random_test, mat2, &res2);

    free(mat1);
    free(mat2);

    printf("res from Determinant CUDA:\t%f\n", res1);
...
void random_complex_matrix_test()
{
    ...
    Cnum res1, res2;
    bool flag_blas = Determinant_cuda_c64(n_random_test, mat1, &res1);
    bool flag_Bareiss = Determinant_Bareiss_c64(n_random_test, mat2, &res2);

    free(mat1);
    free(mat2);

    printf("res from Determinant CUDA:\t%f + %f*I\n", res1.real, res1.imag);
